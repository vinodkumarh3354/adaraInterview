...
const Features = ({ id, header, items, columns = 3, hasBackground = false }: FeaturesProps) => (
  <WidgetWrapper id={id ? id : ''} hasBackground={hasBackground} containerClass="scroll-mt-16 max-w-6xl">
    {header && <Headline header={header} titleClass="text-3xl md:text-4xl" subtitleClass='text-sm' />}
    <ItemGrid
      id={id}
      items={items}
      columns={columns}
      ...
      titleClass="mb-3 md:text-lg font-bold"
      descriptionClass="text-gray-600 text-sm dark:text-slate-400"
      actionClass="hidden"
    />
  </WidgetWrapper>
);
